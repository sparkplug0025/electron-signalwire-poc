...
export const Video = ({
  ...
  const setupRoom = async () => {
    ...
    try {
      // Get room token
      const res = await axios.post("http://localhost:3000/api/get-token", {
      ...
      room.on("room.joined", async (e: VideoRoomSubscribedEventParams) => {
      ...
      const camChangeWatcher = await SignalWire.WebRTC.createDeviceWatcher({
        targets: ["camera"],
      });
      ...
      const micChangeWatcher = await SignalWire.WebRTC.createDeviceWatcher({
        targets: ["microphone"],
      });
      ...
      const speakerChangeWatcher = await SignalWire.WebRTC.createDeviceWatcher({
        targets: ["speaker"],
      });
