...
const baseURL = process.env.SPACE;

console.info(auth, baseURL);
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  switch (req.method) {
    ...
    case "GET":
      return res.json({ info: "get sw token" });
    default:
...
const setupRoomToken = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(200).json({ token, roomExpiration: roomData?.remove_at });
  } catch (e) {
    console.log(e);
